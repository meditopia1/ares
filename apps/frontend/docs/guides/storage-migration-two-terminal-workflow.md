...
The source of truth is the live hosted Supabase project `ldygmpaipxbokxzyzyti`.
The local `day1main` Postgres container is not part of the migration source.
...
Run this from the repo root on your workstation:
...
- Do not point `SOURCE_DB_URL` at the local `day1main` Postgres container; that database is only for local dev and is empty.
