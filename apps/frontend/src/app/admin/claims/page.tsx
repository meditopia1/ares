...
export default function AdminClaimsPage() {
  ...
  const fetchClaims = async () => {
    try {
      ...
      const response = await authFetch(`/api/admin/claims${query ? `?${query}` : ''}`, {
        cache: 'no-store',
      });
    ...
    <SidebarLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Administrative oversight of claims processing</p>
        </div>
          ...
          <CardContent>
            ...
            {loading ? (
              <div className="py-12 flex flex-col items-center justify-center text-gray-600">
                ...
                <p className="mt-3 text-sm">Loading claims...</p>
              </div>
            ) : claims.length === 0 ? (
