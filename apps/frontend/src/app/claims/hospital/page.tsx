...
type ColumnKey = keyof Omit<RegisterRow, 'id' | 'sourceClaim'>;
...
const registerColumns: RegisterColumn[] = [
  { key: 'authNumber', label: 'Auth Number', width: 150, frozen: true },
  { key: 'reportedDate', label: 'Date of Claim reported / received', width: 190, format: 'date', frozen: true },
  { key: 'dol', label: 'DOL', width: 120, format: 'date', frozen: true },
...
function getMonthKey(row: RegisterRow) {
  const date = new Date(row.reportedDate || row.dol);
...
export default function HospitalClaimsPage() {
  ...
  const fetchClaims = async () => {
    try {
      ...
      const response = await authFetch('/api/claims-assessor/queue');
      if (!response.ok) {
        throw new Error('Failed to fetch hospital claims');
      }
      const data = await response.json();
      setClaims((data.claims || []).filter(isHospitalClaim));
    } catch (error) {
  ...
  const registerRows = useMemo(() => [...draftRows, ...claims.map(toRegisterRow)], [claims, draftRows]);
  ...
  const stats = useMemo(() => {
    const openRows = registerRows.filter((row) => !['Paid', 'Repudiated'].includes(row.status));
    const outstandingValue = openRows.reduce((sum, row) => sum + row.outstanding, 0);
    const totalIncurred = registerRows.reduce((sum, row) => sum + row.totalIncurred, 0);

    return {
      openClaims: openRows.length,
      todaysClaims: registerRows.filter((row) => formatDate(row.reportedDate) === new Date().toLocaleDateString('en-ZA')).length,
      outstandingValue,
      awaitingDocumentation: registerRows.filter((row) => row.status === 'Pending Documentation').length,
      readyForPayment: registerRows.filter((row) => row.status === 'Ready for Payment').length,
      paidToday: registerRows.filter((row) => row.status === 'Paid').length,
      hospitalClaims: registerRows.length,
      totalIncurred,
    };
  }, [registerRows]);
          ...
          <div className="space-y-4 min-w-0">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  Showing {filteredRows.length} claims in an Excel-style workspace
                </CardDescription>
              </CardHeader>
              <CardContent>
                {loadingClaims ? (
                  ...
                  <div className="max-h-[calc(100vh-310px)] overflow-auto rounded-md border border-gray-200">
                    <table className="border-separate border-spacing-0 text-xs" style={{ minWidth: activeColumns.reduce((sum, column) => sum + column.width, 0) }}>
                      <thead className="sticky top-0 z-20">
                        <tr>
                          {activeColumns.map((column, index) => (
                            <th
                              key={column.key}
                              className={`border-b border-r border-gray-200 bg-gray-100 px-3 py-2 font-semibold text-gray-700 ${cellAlignment(column)} ${column.frozen ? 'sticky z-30' : ''}`}
                              style={{
                                width: column.width,
                      ...
                      <tbody>
                        {Object.entries(groupedRows).map(([month, rows]) => {
                              ...
                              {!isCollapsed &&
                                rows.map((row) => (
                                  <tr
                                    ...
                                    className={`${rowTint(row.status)} cursor-pointer hover:bg-emerald-50`}
                                  >
                                    {activeColumns.map((column, index) => (
                                      <td
                                        key={`${row.id}-${column.key}`}
                                        className={`border-b border-r border-gray-200 px-3 py-2 align-top text-gray-800 ${cellAlignment(column)} ${column.frozen ? 'sticky z-10 shadow-[1px_0_0_#e5e7eb]' : ''}`}
                                        style={{
                                          width: column.width,
                                          minWidth: column.width,
                                          left: column.frozen ? frozenLeft(activeColumns, index) : undefined,
            ...
            <div className="h-full w-full max-w-xl overflow-y-auto bg-white shadow-2xl">
              ...
              <div className="space-y-5 p-6">
                <DrawerSection title="Register Details">
                  <DetailGrid row={selectedRow} keys={['authNumber', 'reportedDate', 'dol', 'status', 'hospital', 'cause', 'plan', 'icd10']} />
                </DrawerSection>
                <DrawerSection title="Financials">
                  <DetailGrid row={selectedRow} keys={['totalIncurred', 'finalisedPaid', 'outstanding', 'actualCosts', 'memberCosts', 'repudiation']} />
                </DrawerSection>
                <DrawerSection title="Member and Patient">
                  <DetailGrid row={selectedRow} keys={['memberNumber', 'surname', 'initials', 'patientName', 'principalId', 'gender', 'patientDob', 'relationship']} />
                </DrawerSection>
...
function DetailGrid({ row, keys }: { row: RegisterRow; keys: ColumnKey[] }) {
  return (
      ...
      {keys.map((key) => {
        ...
        return (
          <div key={key} className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
            <p className="text-xs text-gray-500">{column.label}</p>
            <div className="mt-1 text-sm font-medium text-gray-900">{formatCell(row, column)}</div>
          </div>
        );
      })}
    </div>
